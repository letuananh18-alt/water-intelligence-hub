...
// Global Handler Functions for N8N Webhook Settings
window.saveN8nWebhook = function() {
  const inputN8n = document.getElementById('n8nWebhookUrlInput');
  const statusN8n = document.getElementById('n8nWebhookStatus');
  if (inputN8n) {
    const val = inputN8n.value.trim();
    ...
    if (statusN8n) {
      if (val) {
        statusN8n.innerHTML = `<span style="color: #0284c7; display: inline-flex; align-items: center; gap: 4px;">🔗 Đã kích hoạt n8n AI Chatbot Webhook (${val})</span>`;
      } else {
        statusN8n.innerHTML = `<span style="color: #64748b;">ℹ️ Chưa kết nối Webhook n8n (AI Assistant đang chạy Engine AI nội bộ)</span>`;
      }
    }
...
window.testN8nConnection = async function() {
  ...
  try {
    ...
    if (btnTestN8n) {
      btnTestN8n.disabled = false;
      btnTestN8n.textContent = "⚡ Test Kết Nối";
    }
  }
};
